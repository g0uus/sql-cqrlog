...
--
-- SELECT
--   GROUP_CONCAT(CONCAT("SUM(CASE WHEN band = '", REPLACE(band, "'", "''"), "' THEN 1 ELSE 0 END) AS `", band, "`") ORDER BY band SEPARATOR ', ')
-- INTO @cols
...
-- SELECT
--   country,
--   COUNT(*) FILTER (WHERE band = '160m') AS "160m",
--   COUNT(*) FILTER (WHERE band = '80m')  AS "80m",
--   COUNT(*) FILTER (WHERE band = '40m')  AS "40m"
-- FROM view_cqrlog_main_by_qsodate
